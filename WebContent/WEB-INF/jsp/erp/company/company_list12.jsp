			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="company/list12.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								<td>
									<div class="nav-search">
										<span class="input-icon">
											<input type="text" placeholder="编号/姓名/手机/职称" class="nav-search-input" id="nav-search-input" autocomplete="off" name="keywords" value="${pd.keywords }" placeholder="输入编号或手机关键词"/>
...
<%-- 未开放批量删除功能				    <c:if test="${QX.del == 1 }">
	...
	<script type="text/javascript">
		...
		//删除
		function del(Id){
			bootbox.confirm("确定要删除该专家信息?", function(result) {
				if(result) {
					top.jzts();
		...
		function random(){
			bootbox.confirm("即将随机抽取一位专家!", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>company/random3.do?random=12";
					$.get(url,function(data){
						if("success" == data.result){
							top.hangge();
							bootbox.dialog({
								message: "<span class='bigger-110'>专家库为空，请添加专家!</span>",
								buttons: 			
								{
									"button" :
									{
							...
							top.hangge();
							bootbox.dialog({
								message: "<span class='bigger-110'>被抽取的专家姓名为："+data.name+"</span>",
								buttons: 			
								{
									"button" :
									{
